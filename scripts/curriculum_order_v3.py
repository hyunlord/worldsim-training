...
CURRICULUM_STAGES_V3: dict[int, set[str]] = {
    1: {"E", "F", "I", "J"},
    2: {"K", "L", "O", "P"},
    3: {"A", "B", "C", "Q"},
    4: {"G", "H", "M", "N", "R", "S", "T", "NEG", "GEN"},
}


def curriculum_order_v3(rows: list[dict], seed: int = 42) -> list[dict]:
    rng = random.Random(seed)
    ordered: list[dict] = []
    consumed_ids: set[int] = set()

    for stage_num in sorted(CURRICULUM_STAGES_V3):
        stage_tasks = CURRICULUM_STAGES_V3[stage_num]
        by_task: dict[str, list[tuple[int, dict]]] = defaultdict(list)
        for index, row in enumerate(rows):
            task = str(row.get("task", ""))
            if task in stage_tasks:
                by_task[task].append((index, row))
        for task in by_task:
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Reorder a v3 training dataset into the v3 curriculum.")
...
def main() -> None:
    ...
    output_path = (
        ...
        if args.output
        else input_path.with_name(f"{input_path.stem}_curriculum.jsonl")
    )
    rows = read_jsonl(input_path)
    ordered = curriculum_order_v3(rows, seed=args.seed)
